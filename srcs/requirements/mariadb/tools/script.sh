...
ALTER USER 'root'@'localhost' IDENTIFIED by '$(cat $MARIA_DB_ROOT_PASSWD)';
CREATE DATABASE ${MARIA_DB_NAME} CHARACTER SET utf8 COLLATE utf8_general_ci;
CREATE USER '${MARIA_DB_USER}'@'%' IDENTIFIED by '$(cat $MARIA_DB_USER_PASSWD)';
GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost' IDENTIFIED BY '$(cat $MARIA_DB_ROOT_PASSWD)';
